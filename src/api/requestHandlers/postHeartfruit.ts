  ...
  async (clientRequest, clientResponse) => {
    ...
    try {
      ...
      if (cooldownRemaining > 0) {
        ...
        voodoo.command({ accountId, command: `player set-stat ${accountId} nightmare 1` });
        voodoo.command({ accountId, command: `player message ${accountId} "A sense of dread overcomes you" 3` });
