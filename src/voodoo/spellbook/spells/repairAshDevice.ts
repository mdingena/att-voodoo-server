...
export const repairAshDevice: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  ...
  const repairedApparatus = repairMaterial(hiltedApparatus, MaterialHash.Ash, repairAmount);
  ...
  voodoo.logger.success(`[${serverName ?? serverId} | ${name}] cast Repair Ash Device`);
};
