...
export const repairRedIronDevice: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  ...
  const repairedApparatus = repairMaterial(hiltedApparatus, PhysicalMaterialPartHash.RedIronAlloy, repairAmount);
  ...
  console.log(`[${serverName ?? serverId} | ${name}] cast Repair Red Iron Device`);
};
