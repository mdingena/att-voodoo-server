...
export const repairIronDevice: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  const hiltedApparatus = voodoo.players[accountId].incantations[0].decodedString.prefab;

  const upgrades = voodoo.getSpellUpgrades({ accountId, spell: 'repairIronDevice' });
  ...
  const repairedApparatus = repairMaterial(hiltedApparatus, MaterialHash.Iron, repairAmount);
