...
export const repairWalnutDevice: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  ...
  const repairedApparatus = repairMaterial(hiltedApparatus, MaterialHash.Walnut, repairAmount);
  ...
  voodoo.logger.success(`[${serverName ?? serverId} | ${name}] cast Repair Walnut Device`);
};
