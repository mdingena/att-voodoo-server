...
export const raiseWalnutTree: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  const upgrades = voodoo.getSpellUpgrades({ accountId, spell: 'raiseWalnutTree' });
  ...
  const tree = createRandomTree(SpeciesHash.Walnut, terminationRate, mainHand.position);
  ...
  console.log(`[${serverName ?? serverId} | ${name}] cast Raise Walnut Tree`);
};
