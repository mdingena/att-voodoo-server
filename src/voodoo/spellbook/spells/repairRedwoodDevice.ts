...
export const repairRedwoodDevice: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  const upgrades = voodoo.getSpellUpgrades({ accountId, spell: 'repairRedwoodDevice' });
  ...
  const repairedApparatus = repairMaterial(hiltedApparatus, PhysicalMaterialPartHash.Redwood, repairAmount);
