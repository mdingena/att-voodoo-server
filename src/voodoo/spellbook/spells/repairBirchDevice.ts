...
export const repairBirchDevice: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  const hiltedApparatus = voodoo.players[accountId].incantations[0].decodedString.prefab;

  const upgrades = voodoo.getSpellUpgrades({ accountId, spell: 'repairBirchDevice' });
  ...
  const repairedApparatus = repairMaterial(hiltedApparatus, PhysicalMaterialPartHash.Birch, repairAmount);
