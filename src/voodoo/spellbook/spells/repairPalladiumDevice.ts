...
export const repairPalladiumDevice: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  const hiltedApparatus = voodoo.players[accountId].incantations[0].decodedString.prefab;

  const upgrades = voodoo.getSpellUpgrades({ accountId, spell: 'repairPalladiumDevice' });
  ...
  const repairedApparatus = repairMaterial(hiltedApparatus, PhysicalMaterialPartHash.CarsiAlloy, repairAmount);
