...
export const repairSilverDevice: SpellFunction = async (voodoo, accountId, upgradeConfigs) => {
  ...
  const repairedApparatus = repairMaterial(hiltedApparatus, MaterialHash.Silver, repairAmount);
  ...
  voodoo.logger.success(`[${serverName ?? serverId} | ${name}] cast Repair Silver Device`);
};
